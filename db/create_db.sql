...
create table bookCategory(
	book varchar(20) references book(id) on delete cascade on update cascade,
    category varchar(50) references category(name) on delete cascade on update cascade,
    primary key (book,category)
);

create table edition(
	id varchar(20) references book(id) on delete cascade on update cascade,
    number int,
...
create table authorWrite(
	author varchar(20) references author(id) on delete cascade on update cascade,
    number int,
    book varchar(20),
...
create table customerOrder(
	id varchar(20) primary key,
    totalCost double not null default 0 check(totalCost>=0),
    orderTime datetime,
    -- orderTime datetime not null check(orderTime<=now()),
    totalDiscount double default 0 check (totalDiscount>=0),
    customer varchar(20) not null references customer(id) on delete cascade on update cascade,
    status boolean not null default false -- false mean that the customer hasn't paid yet
);

create table physicalOrder(
	orderID varchar(20) primary key references customerOrder(id) on delete cascade on update cascade,
...
create table fileOrderContain(
	number int,
    book varchar(20),
    orderID varchar(20) references fileOrder(orderID) on delete cascade on update cascade,
...
create table physicalOrderContain(
	number int,
    book varchar(20),
    orderID varchar(20) references physicalOrder(orderID) on delete cascade on update cascade,
    primary key(number,book,orderID),
...
create table discountApply(
	orderID varchar(20) references customerOrder(orderID) on delete cascade on update cascade,
